...
class DataPreparer:
    ...
    def prep_data(self) -> None:
        ...
        df_home["DID_HOME_WIN"] = [
            1 if (row["winner"] == 1 and row["home"] == 1) else 0 for _, row in df_home.iterrows()
        ]
...
def main() -> None:
    data_preparer = DataPreparer(
        "./unprocessed_data/CFB_GAME_DATA_2013_2023.csv", "TRAINING_DATA.csv"
    )
    data = data_preparer.get_data()
    data.drop(columns=["team", "team_away", "gameID"], inplace=True)
